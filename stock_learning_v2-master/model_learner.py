...
class ModelLearner:
    ...
    def load_dnn_model(self, data: pd.DataFrame, pred_days=120, corp_code='', time_steps=32, columns=None, **params):
        """
        LSTM 모델로 학습을 한다.
        :param data: 학습 데이터
    ...
    def get_best_model_path(self, corp_code):
        ...
        :return:
        """
        return os.path.join(self.RESULTS_PATH, "models", "{}.h5".format(corp_code))
    ...
    def get_unreliable_model_path(self, corp_code):
        ...
        :return:
        """
        return os.path.join(self.RESULTS_PATH, "models", "unreliable", "{}.h5".format(corp_code))
    ...
    def get_try_model_path(self, corp_code):
        ...
        :return:
        """
        return os.path.join(self.RESULTS_PATH, "models", "try", "{}.h5".format(corp_code))
    ...
    def predict_next_by_dnn(self, data: pd.DataFrame, model=None, scalers: tuple = None, corp_code='',
                            **params):
        """
        LSTM 모델로 다은 날을 예측한다.
        :param data: 학습 데이터
        ...
        columns = self.get_dict_value(params, "columns", ["close"])
        converter = DataConverter()
        x = converter.get_last_dataset(data, columns, time_steps, scalers=scalers)
        if model is None:

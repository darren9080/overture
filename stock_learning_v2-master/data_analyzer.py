...
class DataAnalyzer:
    ...
    def predicts_next(self, corps: pd.DataFrame, buy_min_ratio=10, sell_min_ratio=20, **params):
        next_data = []
        ...
        if buy_min_ratio != 0 or sell_min_ratio != 0:
            ...
            worst = next_data.query(f"ratio<={-sell_min_ratio}")
            best_label = pd.DataFrame([{"rank": None, "code": None, "name": "BUY"}])
            top_data = best_label.append(best, ignore_index=True)
            worst_label = pd.DataFrame([{"rank": None, "code": None, "name": "SELL"}])
            top_data = top_data.append(worst_label, ignore_index=True)
            top_data = top_data.append(worst, ignore_index=True)
            next_label = pd.DataFrame([{"rank": None, "code": None, "name": "PREDICT"}])
            top_data = top_data.append(next_label, ignore_index=True)
            next_data = top_data.append(next_data, ignore_index=True)
